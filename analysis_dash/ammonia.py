...
class Ammonia:
    def __init__(self, system_id: int) -> NoReturn:
        ...
        self.str_no_ammonia: str = self.system.no_ammonia
    ...
    def generate_ener_figure(self) -> plotly.subplots.make_subplots:
        ...
        time = self.load_ener_from_database()["time"] - 500
        fig.add_trace(
    ...
    def process_vde_data(self, vde_type):
        if vde_type == "MP2":
            file = f'ammonia-{self.str_no_ammonia}-{self.id_cluster}-MP2.txt'
        elif vde_type == "B3LYP":
            file = f'ammonia-{self.str_no_ammonia}-{self.id_cluster}-B3LYP.txt'
    ...
    def generate_vbe_fig(self):
        try:
            data_mp2 = self.process_vde_data("MP2")
            data_b3lyp = self.process_vde_data("B3LYP")
            fig = plotly.subplots.make_subplots(rows=1, cols=1)
            fig.add_trace(
                go.Scatter(x=data_mp2['frames'], y=data_mp2['VDE'], name="<b>Test</b>",
                           mode='lines', line=dict(color='blue', width=1),
                           hovertemplate='Time: %{x} fs<br>VDE: %{y} eV'),

            )
            fig.add_trace(
                go.Scatter(x=data_b3lyp['frames'], y=data_b3lyp['VDE'], name="<b>Test 2</b>",
                           mode='lines', line=dict(color='red', width=1),
                           hovertemplate='Time: %{x} fs<br>Another Data: %{y} eV'),
            )
    ...
    def __str__(self) -> str:
        ...
        :return:
        """
        return f"Ammonia {self.str_no_ammonia}-{self.id_cluster}"
